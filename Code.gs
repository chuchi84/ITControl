// ═══════════════════════════════════════════════════════════════════════
//  ITControl Pro — Google Apps Script Backend v1.0
...
const COLS = {
  ...
  equipos:      ['id','co','local_id','compra_id','tipo','marca','modelo','serie','estado','specs','campos','software','comentarios'],
  compras:      ['id','co','local_id','proveedor','rut_prov','tipo_doc','num_doc','fecha','quien_pago','forma_pago','monto','estado_pago','cobrado_cliente','descripcion','equipos_ids','adjunto','adjunto_nombre'],
  historial:    ['id','eq_id','co','local_id','tipo','fecha','tecnico','costo','desc','prox'],
  asignaciones: ['id','eq_id','co','sede_id','ubicacion','persona','cargo','fecha','fecha_dev','estado','obs'],
...
function handleRequest(e) {
  try {
    ...
    // Ping — sin auth
    if (action === 'ping') return jsonResponse({ ok: true, version: '1.0', app: 'ITControl Pro' });
