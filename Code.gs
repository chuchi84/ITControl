...
const ACTION_MIN_ROLE = {
  ...
  'email_notify':       'lector',
  'ia_scan':            'lector',
  // adjunto_download / upload / delete: solo técnico o superior; lectura NO
  // ve adjuntos ni los descarga.
  'adjunto_download':   'tecnico',
  // log_add: escritura auxiliar en el log. Se restringe a técnico o superior
  // para que un lector no pueda ensuciar la auditoría con entradas falsas.
  'log_add':            'tecnico',
...
const COLS = {
  empresas:     ['id','name','rut','prefix','counter','counterCompra','counterCompraAnio','giro','contacto','email','tel','dir','sedes','logo','url_base'],
  ...
  solicitudes_soporte:['id','co','nombre','email','equipo_id','titulo','prioridad','desc','estado','fecha','respuesta','tomado_en','resuelto_en','tarea_id','mensajes'],
...
function handleRequest(e) {
  try {
    ...
    switch (action) {
      ...
      case 'empresa_del':
        return jsonResponse(deleteEmpresa(body.id, user));
      ...
      // ── PERSONAS (directorio por empresa) ──
      case 'persona_add':
        return jsonResponse(addRow(HOJAS.personas, COLS.personas, body.persona, user));
      case 'persona_edit':
        return jsonResponse(editRow(HOJAS.personas, COLS.personas, body.persona, user));
      case 'persona_del':
        return jsonResponse(delRow(HOJAS.personas, COLS.personas, body.id, user));
...
function handlePublicRequest(action, body) {
  if (action === 'solicitud_publica') {
    ...
    // Rate limiting: evita que un bot o un usuario mal intencionado inunde la sheet.
    // Se aplica por email (dato disponible sin login). Un mismo email no puede
    // enviar más de 5 solicitudes en 10 minutos, ni más de 20 en 1 hora.
    // Se usa CacheService (memoria de Apps Script, no toca la sheet).
    const rlKey = 'rl_' + String(solic.email).toLowerCase().trim().slice(0,160);
    const cache = CacheService.getScriptCache();
    const contadores = JSON.parse(cache.get(rlKey) || '{"c10":0,"c60":0}');
    if (contadores.c10 >= 5) {
      return { ok: false, error: 'Has enviado demasiadas solicitudes. Intenta de nuevo en unos minutos.' };
    }
    if (contadores.c60 >= 20) {
      return { ok: false, error: 'Límite horario alcanzado. Contacta directamente a soporte por otro medio.' };
    }
    contadores.c10++; contadores.c60++;
    // Ventana corta 10 min (600s) y larga 1h (3600s) — dos claves separadas
    cache.put(rlKey, JSON.stringify(contadores), 600);
    const rlKeyH = rlKey + '_h';
    const contH = JSON.parse(cache.get(rlKeyH) || '{"c60":0}');
    contH.c60 = (contH.c60||0) + 1;
    cache.put(rlKeyH, JSON.stringify(contH), 3600);
    if (contH.c60 > 20) {
      return { ok: false, error: 'Límite horario alcanzado. Contacta directamente a soporte por otro medio.' };
    }
...
function loadAll(user) {
  ...
  return {
    ...
    personas:             scope(sheetToArray(ss, HOJAS.personas,             COLS.personas)),
    log:                  log,
  };
}
...
// Elimina una empresa y TODOS sus datos asociados (equipos, compras, etc.).
// Solo titular/superadmin (ver ACTION_MIN_ROLE). Borra por la columna 'co'.
function deleteEmpresa(id, user) {
  return withLock(function() {
    if (!id) return { ok: false, error: 'Sin ID' };
    const ss = SpreadsheetApp.openById(SHEET_ID);

    // Quitar la empresa del mapa
    const empresas = loadEmpresas(ss);
    if (!empresas[id]) return { ok: false, error: 'Empresa no encontrada' };
    delete empresas[id];
    saveEmpresas(ss, empresas);

    // Borrar filas con co === id en todas las hojas de datos
    const hojasData = [
      HOJAS.equipos, HOJAS.compras, HOJAS.historial, HOJAS.asignaciones,
      HOJAS.licencias, HOJAS.tareas, HOJAS.usuarios, HOJAS.bajas,
      HOJAS.programas_mant, HOJAS.solicitudes_soporte, HOJAS.sesiones_inventario,
      HOJAS.accesorios, HOJAS.personas,
    ];
    hojasData.forEach(nombre => {
      const sh = ss.getSheetByName(nombre);
      if (!sh || sh.getLastRow() <= 1) return;
      const cols = COLS[Object.keys(HOJAS).find(k => HOJAS[k] === nombre)] || [];
      const coIdx = cols.indexOf('co');
      if (coIdx < 0) return;
      const rows = sh.getDataRange().getValues();
      for (let i = rows.length - 1; i >= 1; i--) {
        if (String(rows[i][coIdx]) === String(id)) sh.deleteRow(i + 1);
      }
    });

    _auditInterno(user, 'Empresas', 'eliminar', id, id);
    return { ok: true };
  });
}
...
function saveEmpresa(empresa, user) {
  ...
  // Validaciones de tamaño/formato en campos "abiertos" — espejo del frontend
  // pero forzadas en el servidor por si alguien salta el UI.
  if (empresa.logo) {
    // ~700 KB de base64 ≈ 500 KB de archivo original (base64 crece ~33%)
    if (String(empresa.logo).length > 700 * 1024) {
      return { ok: false, error: 'El logo excede el tamaño permitido (500 KB)' };
    }
    // Debe ser data-URI de imagen o quedar vacío
    if (!/^data:image\/(png|jpe?g|svg\+xml|webp|gif);base64,/.test(String(empresa.logo))) {
      return { ok: false, error: 'Formato de logo no válido' };
    }
  }
  if (empresa.url_base) {
    const u = String(empresa.url_base);
    if (u.length > 300) return { ok: false, error: 'URL base demasiado larga' };
    if (!/^https?:\/\//.test(u)) return { ok: false, error: 'La URL base debe empezar con http:// o https://' };
  }
...
function addLog(entrada, user) {
  ...
  // Cap de tamaño en campos libres — evita payloads inflados / spam.
  const cap = (v, n) => String(v == null ? '' : v).slice(0, n);
  sh.appendRow([
    Date.now(), co, user.email,
    cap(entrada.accion,  120),
    cap(entrada.modulo,  60),
    cap(entrada.detalle, 500),
...
// ── LIMPIEZA DE DATOS (para pruebas desde cero) ───────────────────────
// Ejecuta estas funciones DESDE EL EDITOR de Apps Script (no desde la app).
// Vacían las filas de datos pero CONSERVAN los encabezados (fila 1).
//
// Antes de borrar, hace un respaldo automático a Drive por si acaso.

// Borra SOLO datos operativos. Conserva Empresas, Usuarios y Acceso.
function limpiarDatosPrueba() {
  respaldoDiario(); // respaldo de seguridad
  const hojas = [
    HOJAS.equipos, HOJAS.compras, HOJAS.historial, HOJAS.asignaciones,
    HOJAS.licencias, HOJAS.tareas, HOJAS.bajas, HOJAS.programas_mant,
    HOJAS.solicitudes_soporte, HOJAS.sesiones_inventario, HOJAS.accesorios,
    HOJAS.log,
  ];
  _vaciarHojas(hojas);
  Logger.log('✅ Datos operativos borrados. Empresas, Usuarios y Acceso se conservaron.');
}

// Borra TODO menos la hoja Acceso (para no perder el ingreso). También vacía
// Empresas y Usuarios: partir totalmente de cero.
function limpiarTodoSalvoAcceso() {
  respaldoDiario();
  const hojas = [
    HOJAS.equipos, HOJAS.compras, HOJAS.historial, HOJAS.asignaciones,
    HOJAS.licencias, HOJAS.tareas, HOJAS.bajas, HOJAS.programas_mant,
    HOJAS.solicitudes_soporte, HOJAS.sesiones_inventario, HOJAS.accesorios,
    HOJAS.log, HOJAS.empresas, HOJAS.usuarios,
  ];
  _vaciarHojas(hojas);
  Logger.log('✅ Todo borrado salvo Acceso. Recuerda crear una empresa antes de usar la app.');
}

function _vaciarHojas(nombres) {
  const ss = SpreadsheetApp.openById(SHEET_ID);
  nombres.forEach(nombre => {
    const sh = ss.getSheetByName(nombre);
    if (sh && sh.getLastRow() > 1) {
      sh.deleteRows(2, sh.getLastRow() - 1);
      Logger.log('  vaciada: ' + nombre);
    }
  });
}
